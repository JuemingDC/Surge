...
WeTalk 签到任务 = type=cron, cronexp="20 8,20 * * *", script-path=https://raw.githubusercontent.com/你的用户名/你的仓库/main/WeTalk.Surge.js, timeout=120
...
function saveStore(store) {
  writeStore(JSON.stringify(store), storeKey);
}
...
function buildUA(baseUA, seed) {
  ...
  return 'WeTalk/30.6.0 (com.innovationworks.wetalk; build:28; iOS '
    + iosVer
    + ') Alamofire/5.4.3';
}
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!email) {
    notify(
      '⚠️ 抓取失败',
      '请求里未取到 email 参数，无法识别账号。请确认已登录后再触发抓包。'
    );

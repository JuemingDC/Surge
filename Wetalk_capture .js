/**
 * WeTalk_capture for Surge.js
 * WeTalk Surge 专用抓取脚本
 * 存储键值: wetalk_accounts_v1
 * 说明: 存储结构保持与 WeTalk_task.js 兼容
 */
...
function captureAccount() {
  ...
  const baseUA = getHeader(headers, 'User-Agent');
  const now = Date.now();
  ...
  const uaSeed = existed ? store.accounts[accountId].uaSeed : store.order.length;

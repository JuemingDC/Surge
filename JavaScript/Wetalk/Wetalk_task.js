...
function migrateStore(store) {
  ...
  (store.order || Object.keys(store.accounts)).forEach(oldId => {
    ...
    const paramsRaw = acc.capture && acc.capture.paramsRaw ? acc.capture.paramsRaw : {};
    const email = emailKeyOf(paramsRaw);
    const newId = email || fingerprintOf(paramsRaw) || oldId;
...
function runAccount(acc, index, total) {
  const tag = `[参数${index + 1}/${total} ${acc.alias || acc.email || acc.id}]`;
          ...
          fetchApi('videoBonus')
            .then(res => {
              try {
                ...
                if (d.retcode === 0) {
                  const bonus = d.result && d.result.bonus ? d.result.bonus : '?';
  ...
  return fetchApi('queryBalanceAndBonus')
    ...
    .then(res => {
      try {
        ...
        if (d.retcode === 0) {
          const bonusHint = ((d.result && d.result.bonusHint) || d.retmsg || '').replace(/\n/g, ' ');
          msgs.push(`✅ 签到：${bonusHint}`);
        } else {
...
const ids = store.order.filter(id => {
  const acc = store.accounts[id];
  return acc && acc.capture && acc.capture.paramsRaw;
});

if (!ids.length) {
  notify('⚠️ 未抓到任何参数', '请先打开 WeTalk 触发 queryBalanceAndBonus 抓包。');
  $done();
} else {
  ...
  chain
    .then(() => {
      notify(`全部完成 (${total}组参数)`, results.join('\n———\n'));
      $done();
    })
    .catch(err => {

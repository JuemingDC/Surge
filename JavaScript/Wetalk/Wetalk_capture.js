...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!paramCount) {
    ...
    const alias = existed ? (store.accounts[accId].alias || email || accId) : (email || accId);
    ...
    notify(
      existed ? '账号参数已更新' : '✅ 新参数已入库',
      `${email || accId}\n当前账号/参数总数：${total}`
    );

    console.log(`〖${scriptName}〗${existed ? 'update' : 'add'} ${email || accId}\n${JSON.stringify(store.accounts[accId], null, 2)}`);

    $done({});
  }
} else {
  notify('⚠️ 运行环境错误', '该脚本仅用于 http-request 捕获参数。');
  $done({});
}
